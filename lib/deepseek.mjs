...
// 连通性测试
export async function testConnection({ apiKey, model = DEFAULT_MODEL, baseUrl = DEFAULT_BASE, timeoutMs = 30000 }) {
...
// 非流式对话：优先采用流式管道（stream: true）聚合结果，从根源杜绝代理与 API 网关 504 超时
export async function chat({ apiKey, model = DEFAULT_MODEL, messages, temperature = 0.7, maxTokens = 4096, baseUrl = DEFAULT_BASE, timeoutMs = 180000, jsonMode = false }) {
  // 1. 优先使用流式管道聚合结果（实时获取数据流，避免大模型/推理模型等待完整结果导致网关超时 504）
  try {
    let fullText = '';
    for await (const chunk of chatStream({ apiKey, model, messages, temperature, maxTokens, baseUrl, timeoutMs })) {
      fullText += chunk;
    }
    if (fullText.trim()) {
      return fullText;
    ...
    console.warn(`[chat] 流式收集失败或不支持，自动切换非流式降级请求:`, streamErr.message);
  }
...
export async function* chatStream({ apiKey, model = DEFAULT_MODEL, messages, temperature = 0.7, maxTokens = 8192, baseUrl = DEFAULT_BASE, timeoutMs = 300000 }) {
  ...
  while (true) {
    ...
    while ((idx = buffer.indexOf('\n')) >= 0) {
      ...
      const delta = choice?.delta?.content ?? choice?.delta?.text ?? choice?.text ?? choice?.message?.content ?? choice?.delta?.reasoning_content;
  ...
  // 处理 buffer 中末尾可能未带换行符的最后一条 SSE 数据
